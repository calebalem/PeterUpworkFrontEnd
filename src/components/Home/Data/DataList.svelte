...
<ul
  class=" shadow-lg bg-base-100 rounded-box px-2 flex flex-col h-96 mx-1 text-base-content"
>
  <li class="menu-title flex flex-row shadow-lg rounded-box">
    ...
    <label
      for="add-table-modal"
      class="btn btn-outline btn-primary btn-xs m-1 modal-button"
    >
      <AddFilled16 />
    </label>
    <button class="btn btn-outline btn-primary btn-xs m-1">
      <TrashCan16 />
    </button>
  </li>
  {#each $tableNames as tableName}
    <li class="my-3">
      ...
      <label
        class="text-base-content link link-hover"
        on:click={(event) => selectedTable.set(event.target.textContent)}
        >{tableName}</label
      >
    </li>
  {/each}
...
<div class="modal text-base-content">
  <div class="modal-box h-50 max-h-96">
    <div class="overflow-y-auto  shadow-lg m-2 p-2">
      ...
      <div class="grid grid-cols-12 gap-2">
        <input
          ...
          class="input input-primary input-bordered col-span-12"
        ...
        {#each tableSchemes as tableScheme}
          <input
            ...
            class="input input-primary input-bordered col-span-5 "
            bind:value={tableScheme.columnName}
          />
          <select
            class="select select-bordered select-primary w-full max-w-xs col-span-5"
            bind:value={tableScheme.columnType}
          >
          ...
          <button
            ...
            on:click={addColumn}><AddFilled16 /></button
          >
        {/each}
      </div>
    </div>
    <div class="modal-action">
      ...
      <label for="add-table-modal" class="btn" on:click={cancel}>Close</label>
    </div>
  </div>
</div>
